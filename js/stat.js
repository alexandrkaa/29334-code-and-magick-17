...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var j = 0; j < names.length; j++) {
    ctx.fillStyle = textColor;
    ctx.fillText(Math.round(times[j]), CLOUD_X + GAP + (STRING_WIDTH + GAP) * j, CLOUD_HEIGHT - ((BAR_HEIGHT * times[j]) / maxTime + GAP_UP));
    ctx.fillText(names[j], CLOUD_X + GAP + (STRING_WIDTH + GAP) * j, CLOUD_Y + CLOUD_HEIGHT);
    ctx.fillStyle = getFillColumn(names[j]);
    ctx.fillRect(CLOUD_X + GAP + (STRING_WIDTH + GAP) * j, CLOUD_HEIGHT - ((BAR_HEIGHT * times[j]) / maxTime + GAP_UP), STRING_WIDTH, ((BAR_HEIGHT * times[j]) / maxTime + GAP_UP));
  }
};
